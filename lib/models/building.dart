...
class Door {
  ...
  final List<int> floors; // Added to maintain compatibility with door.dart
...
Building parseBuildingFromJson(Map<String, dynamic> json) {
  ...
  for (final d in doorsList) {
    ...
    final door = Door(
      ...
      side: side,
      floors: floorNum != null
          ? [floorNum]
          : [], // Include floor number in floors list
    );
    ...
    if (floorNum != null && floorsByNumber.containsKey(floorNum)) {
      ...
      if (type == DoorType.other || floorNum == null) {
        unassigned.add(door.copyWithFloor(-1)); // mark as "Main Building"
      } else {
...
/// ---------------------- Door Floor Extension ----------------------
extension DoorFloorCopy on Door {
  Door copyWithFloor(int? floorNumber) {
    final floorId = floorNumber ?? -1; // map null → -1
    return Door(
      id: id,
      name: name,
      type: type,
      inferredFloor: floorNumber,
      side: side,
      floors: [floorId],
    );
  }
